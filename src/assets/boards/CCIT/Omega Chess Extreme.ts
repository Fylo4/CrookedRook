import { Wins, PieceAttributes } from "../../TCR_Core/Constants";

export const ccit_omega_chess_extreme = {
    name: "Omega Chess Extreme",
	author: "Ristet",
    description: "Omega Chess but with Advanced stuff and a bit more!",
	style: "checkered",
	width: 12,
	height: 12,
	wins: [Wins.royal_capture_n, Wins.stalemate],

	all_pieces: [
		{
			name: "Pawn Rabbit",
			description: "Moves passively forward one step (or 2 or 3 on its first move), captures one step forward-diagonally, and promotes when reaching the back rank",
            sprite: "pawn",
            symbol: "p",
            notation: "",
			move: "i[0 1 1 3]Bae+[S]ae+[P]ca",
            promotions: {white: "black_rank_2", black: "white_rank_2", to: ["R", "Q", "K", "B", "J", "K", "S", "C", "W"]},
			attributes: [PieceAttributes.ep_capturer, PieceAttributes.ep_captured],
		},
		{
            name: "Rook",
            description: "Moves sideways and vertically as many spaces as it wants",
            sprite: "rook",
			symbol: "R",
			move: "[R]Ba",
			attributes: [PieceAttributes.castle_to],
		},
		{
            name: "Knighty",
            description: "Moves like a Knight but can passively move like a Zebra",
            sprite: "knight",
			symbol: "N",
			move: "[N]a+[Z]ae",
		},
		{
            name: "Bishop",
            description: "Moves diagonally as many spaces as it wants",
            sprite: "bishop",
			symbol: "B",
			move: "[B]Ba",
		},
		{
            name: "Castling Queen",
            description: "Queen but it can castle like a King",
            sprite: "queen",
			symbol: "Q",
			move: "[Q]Ba+i[1 0 2 -1]BemP{Rook}",
			attributes: [PieceAttributes.castle_from]
		},
		{
            name: "King",
            description: "Moves one step in any direction. Can be checked and checkmated.",
            sprite: "king",
			symbol: "K",
            move: "[K]a+i[1 0 2 -1]BemP{Rook}",
			attributes: [PieceAttributes.royal, PieceAttributes.castle_from],
		},
		{
            name: "Swapper",
            description: "Moves like a Queen, swaps with enemies instead of capturing them",
            sprite: "pylon",
			symbol: "S",
            move: "[Q]Ba",
			attributes: [PieceAttributes.save_enemy, PieceAttributes.dont_flip_enemy],
		},
		{
            name: "Swapper Start",
            description: "Step forward to enter the board. Moves like a Queen, swaps with enemies instead of capturing them",
            sprite: "pylon",
			symbol: "s",
			notation: "S",
            move: "[S]ae",
			promotions: {to: "S", white: "white_rank_2", black: "black_rank_2"},
		},
		{
            name: "Jester",
            description: "Copies the last moved piece AND immobolizes enemies",
            sprite: "jester",
			symbol: "J",
            move: "[K]a",
			attributes: [PieceAttributes.copy_move, PieceAttributes.copy_attrib, PieceAttributes.glue_curse, PieceAttributes.peace_curse, PieceAttributes.no_default_move],
			held_move: "[K]",
		},
		{
            name: "Jester Start",
            description: "Step forward to enter the board. Copies the last moved piece AND immobolizes enemies",
            sprite: "jester",
			symbol: "j",
			notation: "J",
            move: "[S]ae",
			promotions: {to: "J", white: "white_rank_2", black: "black_rank_2"},
		},
		{
            name: "Kamikaze",
            description: "Moves like a King or Knight but it and its capturer/captured will be removed",
            sprite: "potion",
			symbol: "Z",
            move: "([K],[N])a",
			attributes: [PieceAttributes.bomb, PieceAttributes.fireball],
		},
		{
            name: "Kamikaze Start",
            description: "Step forward to enter the board. Moves like a King or Knight but it and its capturer/captured will be removed",
            sprite: "potion",
			symbol: "z",
			notation: "Z",
            move: "[S]ae",
			promotions: {to: "Z", white: "white_rank_2", black: "black_rank_2"},
		},
		{
            name: "Omega Wizard",
            description: "Moves like a Giraffe(Camel) or a Ferz",
            sprite: "sorcerer",
			symbol: "W",
            move: "([F],[C])a",
		},
		{
            name: "Omega Champion",
            description: "Moves like a Crow(Dabbabah), Dog(Wazir), or an Alfil",
            sprite: "guardian",
			symbol: "G",
            move: "([W],[D],[A])a",
		},

	],
    setup: "bW 3. bz bs bj bz 3. bW . bG bR bN bB bQ bK bB bN bR bG 2. 10bp",
    copy: "flip",
	active_squares: "100000000001 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 100000000001",
	zones: [
		"100000000001 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 011111111110 100000000001"
	],
}